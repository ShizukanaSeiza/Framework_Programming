<x-app-layout>
    <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow mt-8">
        ...
        <form action="{{ route('ekyc.step2.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="mb-4">
                <label class="block font-medium mb-1 text-sm text-gray-700 dark:text-gray-300">Foto KTP</label>
                ...
                @endif
                 ...
                 @if ($data && $data->file_ktp)
                <img src="{{ asset('storage/'.$data->file_ktp) }}" class="h-32 mt-2 border">
                @endif
            </div>

            <div class="mb-4">
                <label class="block font-medium mb-1 text-sm text-gray-700 dark:text-gray-300">Selfie Dengan KTP</label>
                ...
                @if ($data && $data->file_selfie)
                <img src="{{ asset('storage/'.$data->file_selfie) }}" class="h-32 mt-2 border">
                @endif
            </div>
