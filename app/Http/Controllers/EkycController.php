...
class EkycController extends Controller
{
    ...
    public function showStep4()
    {
        ...
        $dataa = EkycRegistration::where('user_id', auth()->id())->first();
        $provinsiList = MasterAlamat::select('provinsi')->distinct()->pluck('provinsi');
        $kotaList = [];
        $kecamatanList = [];

        if ($data && $data->provinsi) {
            $kotaList = EkycRegistration::where('provinsi', $data->provinsi)
                ->select('kota')
                ->distinct()
                ->pluck('kota');
        }
        ...
        if ($data && $data->kota) {
            $kecamatanList = EkycRegistration::where('kota', $data->kota)
                ->select('kecamatan')
                ->distinct()
                ->pluck('kecamatan');
        }
    ...
    public function storeStep4(Request $request)
    {
        ...
        return redirect()->route('ekyc.step4')->with('success', 'Data alamat dan informasi tambahan berhasil disimpan.');
    }
}